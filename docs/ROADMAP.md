...
| --- | --- | --- |
| `done` | **AgentDojo evaluation, model-free** | Unblocked by realising AgentDojo publishes attacker *and* correct calls as data, so both security and utility can be replayed without a model. **A trade-off surface, not a number: 96.6%/43.3% to 85.1%/77.3% over thirty measured configurations.** [`BENCHMARKS.md`](BENCHMARKS.md). |
| `done` | **Raise utility past 70%, or record the thesis as disproven** | Utility reached 77.3%; security fell to 85.1%. The best configuration holding ≥90% security is 91.6%/68.0%. The criterion asked for both and was missed. The claim is narrowed to match rather than restated ([`REVIEW.md`](REVIEW.md)). |
...
| `next` | **Path construction** | A path joined from a principal-named leaf and a server-named root traces to nobody. Same shape as computed amounts, with no budget to fall back on. Component-wise attribution is the obvious candidate; it is not built. |
