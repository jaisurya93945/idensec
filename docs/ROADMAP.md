...
| `next` | **Make U08 visible at configuration time** | A field-path grant asserts something about data nobody has seen yet. Today the only signal is the audit log, after the call. A grant that could declare its expected operand *shape* and refuse a surprise would be containment rather than detection; whether that is expressible without becoming a classifier is unknown. |
