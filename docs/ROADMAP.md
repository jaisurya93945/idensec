...
| --- | --- | --- |
| `done` | **AgentDojo evaluation, model-free** | Unblocked by realising AgentDojo publishes attacker *and* correct calls as data, so both security and utility can be replayed without a model. **A trade-off curve: 95.4%/34.0% to 85.1%/76.3%.** [`BENCHMARKS.md`](BENCHMARKS.md). |
| `done` | **Raise utility past 70%, or record the thesis as disproven** | Utility reached 76.3%; security fell to 85.1%. The criterion asked for both and was missed. The claim is narrowed to match rather than restated ([`REVIEW.md`](REVIEW.md)). |
| `next` | **Bind the principal's *reference* to a directory entry** | The one thing that would move the curve. The task says "the networking event"; the directory says which id that is. Binding a reference to an entry is a different mechanism from attribution, and it is the only identified answer to entity selection. **A research problem, not a task** — and if a bounded attempt produces nothing, stopping is the honest option ([`REVIEW.md`](REVIEW.md)). |
