...
| --- | --- | --- |
| `done` | **AgentDojo evaluation, model-free** | Unblocked by realising AgentDojo publishes attacker *and* correct calls as data, so both security and utility can be replayed without a model. **92.0% security, 56.7% utility.** [`BENCHMARKS.md`](BENCHMARKS.md). |
| `next` | **Raise utility past 70%, or record the thesis as disproven** | 56.7% is above PACT's deployed row and far below its oracle row. The number moved 22.7% → 56.7% in one sitting on contract fixes, so it has not converged — but nothing else on this roadmap matters more, and leaving it in a table while building features is the failure mode ([`REVIEW.md`](REVIEW.md)). |
