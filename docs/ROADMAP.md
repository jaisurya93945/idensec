...
| `next` | **A seeded contract corpus** for common MCP servers, versioned and reviewable. | The one candidate moat that grows with adoption ([`COMPETITORS.md`](COMPETITORS.md) §4). Two servers now complete and reviewed — all 14 tools of `server-filesystem` and all 9 of `server-memory`, every effect hand-declared. Five to go, and the cost per server is now known rather than assumed. |
