...
| `next` | **A seeded contract corpus** for common MCP servers, versioned and reviewable. | The one candidate moat that grows with adoption ([`COMPETITORS.md`](COMPETITORS.md) §4). One server is now complete and reviewed — all 14 tools of `server-filesystem`, every effect hand-declared — and the shipped example previously covered 4 of them. Six servers to go, and the cost per server is now known rather than assumed. |
