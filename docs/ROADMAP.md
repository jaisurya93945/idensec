...
| `next` | **Composition beyond paths** | URLs decompose too (scheme, host, path), and so do ARNs and git remotes. Whether the same universal check is sound for them is unmeasured, and a wrong answer on a URL host is worse than on a path segment. |
